...
void control(float leftSensor, float rightSensor) {
  ...
  motorSet(speed - constrain(output/20.0,-0.1,0.1)-abs(constrain(output/25.0,-0.05,0.05))*0.3, left);
  motorSet(speed + constrain(output/20.0,-0.1,0.1)-abs(constrain(output/25.0,-0.05,0.05))*0.3, right);
  ...
  previousError = error/1.01;
  integral=integral/1.015;
}


void motorSet(float power, Motor motor){
  power=power*127.0;
  int8_t motorBits = (power>0)?map(power,0.0,127.0,2.0,127.0):map(power,-127.0,0.0,-127.0,-2.0);  //remove motor power deadzone (values too small don't move)
  if(motor==left){
...
<body>
  <h2>ESP32 Controller Settings</h2>
  Speed: <input type="range" min="0.05" max="0.15" step="0.01" id="speed" value=")RAW" + String(speed) + R"RAW("><span id="speedValue">)RAW" + String(speed) + R"RAW(</span><br>
  Kp: <input type="range" min="0.1" max="10.0" step="0.1" id="kp" value=")RAW" + String(Kp) + R"RAW("><span id="kpValue">)RAW" + String(Kp) + R"RAW(</span><br>
  Kd: <input type="range" min="-1.0" max="1.0" step="0.1" id="kd" value=")RAW" + String(Kd) + R"RAW("><span id="kdValue">)RAW" + String(Kd) + R"RAW(</span><br>
  Ki: <input type="range" min="0.0" max="0.5" step="0.01" id="ki" value=")RAW" + String(Ki) + R"RAW("><span id="kiValue">)RAW" + String(Ki) + R"RAW(</span><br>
  ...
  <script>
    function updateSettings() {
      ...
      fetch(`/set?speed=${speed}&kp=${kp}&kd=${kd}&ki=${ki}`)
    ...
    kpSlider.oninput = function() {
      document.getElementById("kpValue").textContent = this.value;
    }
    kdSlider.oninput = function() {
      document.getElementById("kdValue").textContent = this.value;
    }
    kiSlider.oninput = function() {
      document.getElementById("kiValue").textContent = this.value;
    }
  </script>
</body>
</html>
