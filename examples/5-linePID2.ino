...
const float Ki = 0.0045;  // Integral gain
const float Kd = 0.008;  // Derivative gain
...
void control(float leftSensor, float rightSensor) {
  ...
  float constrained = constrain(output/4.0,-0.4,0.4);

  float leftMotorPower = 0.001;
  float rightMotorPower = 0.001;

  if(constrained>0){
    rightMotorPower += constrained;
  } else {
    leftMotorPower -= constrained;
  }
  
  //motorSet((0.2 + (constrained)<0?-constrained:0.0), left);
  //motorSet((0.2 + (constrained)>0?constrained:0.0), right);

  motorSet(leftMotorPower,left);
  motorSet(rightMotorPower,right);
